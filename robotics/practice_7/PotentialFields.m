function Potential_Fields
    ...
    while(norm(GoalError)>1 && k<nMaxSteps)
	  ...
	  %
            hInfluentialObstacles = plot(Map(1, iInfluencial), Map(2, iInfluencial), 'g*');
            dist_influencial = Distance(iInfluencial);
            dp_influencial = -Dp(:, iInfluencial);
            fi = (-1/RadiusOfInfluence + dist_influencial .^ -1) ...
                .* 1/RadiusOfInfluence^2 .* dp_influencial ./ dist_influencial;
