...
def EKFMappingRob():
    ...
    MapSize = 100
    Map = MapSize * np.random.uniform(low = 0, high = 1, size = (2, nFeatures)) - MapSize / 2

    # Covariances for our very bad & expensive sensor (in the system <d, theta>)
    Sigma_r = 8.0
    Sigma_theta = 7 * np.pi / 180
    Q = np.diag([Sigma_r ** 2, Sigma_theta ** 2]) 

    # Initial robot pose
    xVehicleTrue = get_pose([- MapSize / 1.5, - MapSize / 1.5, 0]) # We know the exact robot pose at any moment
    ...
    # MappedFeatures relates the index of a feature from the true map and its place within the state (which depends on when it was observed)
    MappedFeatures = np.zeros(shape = (nFeatures, 1))

    # Storing the number of times a feature has been seen
    # Also store the handler to the graphical info shown
    ObservedTimes = np.zeros(shape = (nFeatures, 2))

    # Initial graphics - Plot true map
    axis = np.array([- MapSize - 5, MapSize + 5, - MapSize - 5, MapSize + 5])
    fig = plt.figure(1)
    ...
    for k in range(nSteps):
        ...
        if not isNan(MappedFeatures(iFeature)): # Yes, it is in the map already
            #
            # Predict observation
            #
            
            # Find out where it is in the state vector
            FeatureIndex = MappedFeatures[iFeature]

            # xFeature is the current estimation of the position of the landmark "FeatureIndex"
            xFeature = xPred[FeatureIndex : FeatureIndex + 1]

            # Predicts the observation
            zPred = 0000000000 # Hint: use getRangeAndBearing auxiliary_functions

            # Get observation Jacobians
            jHxf = getObsJac(xVehicleTrue, xFeature)
            ...
            S = reduce(np.matmul, [jH, PPred, jH.T]) + QEst
            K = reduce(np.matmul, [PPred, jH.T, np.linalg.inv(S)]) # Gain
            xEst = xPred + np.matmul(K, Innov)
            PEst = PPred - reduce(np.matmul, [K, S, K.T])

            # Ensure P remains symmetric
            PEst = .5 * (PEst + PEst.T)
        else:   # Not in the current map (state)

            #This is a new feature, so add it to the map
            nStates = len(xEst)

            # The observation is in the local frame of the robot, it has to be translated to the global frame
            xFeature = 00000

            # Add it to the current sate
            xEst = np.vstack((xEst, xFeature)) # Each new feature two new rows

            # Compute the jacobian
            jGz = GetNewFeatureJacs(xVehicleTrue, z) # Dimension 2x2

            _top = np.hstack((np.eye(nStates), np.zeros(nStates, 2)))
            _bottom = np.hstack((np.zeros(2, nStates), jGz))
            M = np.vstack((_top, _botom))

            PEst = reduce(np.matmul, [M, blkdiag(PEst, QEst), M.T])
            ...
            MappedFeatures[iFeature] = len(xEst) - 1

        if k % 2 == 0:
            draw_robot(xVehicleTrue, 'r', axis)
...
def PlotNumberOfReadings(xVehicleTrue, iFeature, Map):
    return

def persistent_k():
    persistent_k.counter += 1
persistent_k.counter = 0
...
def getRandomObservationFromPose(x, Map, Q, mode = 'one_landmark'):
