...
showDerivSeq vars dseq = unlines (map showConfig dseq)
  where
    ...
    showConfig config@(Inter ss s) | isAbort config = line ++ "\nStuck state:\n" ++ line
                                   | otherwise = line
                                   where
                                        line = show ss ++ "\n" ++ unlines (showVars s vars)
...
{- Formal definition of 'repeat S until b'
        ...
        [repeat-sos]  ----------------------------------------------------------------------------
                         <repeat S until b, s> -> <S; If b do (Skip) else (repeat S until b), s>
-}
...
derivSeqAbort st ini    | isAbort (Inter st ini) = (Inter st ini) : []
                        | isFinal next = (Inter st ini) : next : []
...
sosAexp (Redex (Add a1 a2) s)         | isValue nextA1 = Redex (Add (N vA1) a2) s
...
showExp0 :: IO ()
showExp0 = showAexpSeq exp0 sExp
